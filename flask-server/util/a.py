...
def _on_connect():
    ...
    if not username:                                     
        flask_app.logger.warning("Rejected connection without username (sid=%s)", sid)  # ← MOD
        return False                                      
    ...
    if old_id:                                            # reconnect – keep same data
        sid_map[sid] = playerKey
        flask_app.logger.info("Re‑connect: %s (user=%s)", playerKey, players[playerKey]["username"])
